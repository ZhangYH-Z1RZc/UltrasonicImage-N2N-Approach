...
class Multiresblock(nn.Module):
    def __init__(self, input_features: int, corresponding_unet_filters: int, alpha: float = 1.67,
                 enable_batchnorm: bool = False) -> None:
        """
            MultiResblock

            Arguments:
              x - input layer
              corresponding_unet_filters - Unet filters for the same stage, equals to parameter U in the original implementation
...
class Conv2d_batchnorm(nn.Module):
    def __init__(self, input_features: int, num_of_filters: int, kernel_size: Tuple = (2, 2), stride: Tuple = (1, 1),
                 activation: str = 'relu', padding: int = 0, enable_batchnorm: bool = False) -> None:
        """
        Arguments:
          x - input layer
...
class Respath(nn.Module):
    def __init__(self, input_features: int, filters: int, respath_length: int, enable_batchnorm: bool = False) -> None:
        """
        Arguments:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x += shortcut
        x = x + shortcut  # inplace operation seems to evoke a training problem
        x = F.relu(x)
        if self.enable_batchnorm:
            x = self.batch_norm1(x)
...
class MultiResUnet(nn.Module):
    def __init__(self, channels: int, filters: int = 32, nclasses: int = 1) -> None:
        ...
        self.conv_final = Conv2d_batchnorm(input_features=self.in_filters9, num_of_filters=self.nclasses,
                                           kernel_size=(1, 1), activation='None', enable_batchnorm=(self.nclasses == 1))
